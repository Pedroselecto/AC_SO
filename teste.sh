...
touch biblioteca.txt
touch _temp_results.txt
...
if [ ! -s "biblioteca.txt" ]; then
		echo "Sist. Oper.|Sistemas Operacionais Modernos|Tanenbaum|DISPONÍVEL" > biblioteca.txt
		echo "Sist. Oper.|O Design do UNIX|Maurice Bach|EMPRESTADO" >> biblioteca.txt
    echo "Cálculo|Cálculo Volume 1|James Stewart|DISPONÍVEL" >> biblioteca.txt
    echo "Cálculo|Cálculo Volume 2|James Stewart|DISPONÍVEL" >> biblioteca.txt
    echo "Programação|Código Limpo|Robert C. Martin|DISPONÍVEL" >> biblioteca.txt
    echo "Programação|Algoritmos: Teoria e Prática|Cormen|EMPRESTADO" >> biblioteca.txt
fi
...
buscar_biblioteca() {
    ...
    if [ ! -f "biblioteca.txt" ]; then
        echo "ERRO: Arquivo 'biblioteca.txt' não encontrado."
        echo "Não é possível realizar a busca."
        sleep 2
        return
    fi
	...
	# Se der enter com o o input em branco, exibe a mesnagem de erro
    ...
    grep -i "$termo_busca" biblioteca.txt > _temp_results.txt

    # Verifica se o arquivo temporário de resultados NÃO está vazio
    if [ -s "_temp_results.txt" ]; then
        echo ""
        (
            echo "ÁREA|TÍTULO|AUTOR|STATUS"
            cat _temp_results.txt
        ) | column -t -s '|'
    else
        echo "Nenhum livro encontrado com o termo '$termo_busca'."
    fi
    
    rm _temp_results.txt # Limpa o arquivo temporário
    echo ""
}
